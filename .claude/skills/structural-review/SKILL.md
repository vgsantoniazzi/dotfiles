...
- **Verdict**, STANDS or NARROWED with its conditions
- **Test that protects the change**, what proves behavior is preserved across the refactor.

Group as **Must-fix structural / Should-fix structural / Nice-to-have**. Be direct and high-conviction; do not soften. Before presenting, one adversarial pass. Spawn ONE `Agent
subagent_type=general-purpose` over the whole set. State in its prompt that it may
not Edit, Write, or run anything outside `git`.

```
These are structural findings. They assert a shape, not a failure. Do not ask
for a reproduction; there is none.

FINDINGS: <numbered: claim, file:line, asserted maintenance cost>

Per finding: quote the code and say whether the structural claim is literally
true of it. Is it the codebase's deliberate convention, and do the UNCHANGED
siblings agree? Is the maintenance cost concrete and paid by someone
identifiable, or speculative scale that does not exist yet? Is the proposed fix
the smallest change, or a rewrite or premature abstraction wearing one?

Verdict: REFUTED (basis false, convention, or speculative), NARROWED
(conditions), or STANDS (what you checked).
```

Drop REFUTED silently, print NARROWED conditions on the finding, and state
"N findings raised, M survived" above the groups. Every finding must tie to THIS
diff with real maintenance impact, not a best-practices list.
